...
const HIDEOUT_INVENTORY_STORAGE = 'hideout_inventory_v2'; // Changed to v2 for new format
...
let kappaSearch = '';
let hideoutStations = [];
let hideoutBuilt = new Set(); // "{stationId}_{level}"
let hideoutItemsInventory = {}; // { itemId: quantity }
let consolidatedHideoutItems = []; // Lista de items únicos del refugio
let hideoutItemsFilter = 'all';
let hideoutItemsSearch = '';
let hideoutCurrentView = 'stations'; // 'stations' o 'items'
...
// ═══════ STORAGE ═══════
function saveKappa(){ localStorage.setItem(KAPPA_STORAGE, JSON.stringify([...kappaFound])); }
function loadKappa_storage(){ try { const s = localStorage.getItem(KAPPA_STORAGE); if(s) kappaFound = new Set(JSON.parse(s)); } catch{} }
function saveHideout(){ localStorage.setItem(HIDEOUT_STORAGE, JSON.stringify([...hideoutBuilt])); }
...
function saveHideoutInventory(){ localStorage.setItem(HIDEOUT_INVENTORY_STORAGE, JSON.stringify(hideoutItemsInventory)); }
function loadHideoutInventory_storage(){ 
  try { 
    const s = localStorage.getItem(HIDEOUT_INVENTORY_STORAGE); 
    if(s) {
      const parsed = JSON.parse(s);
      // If old format (Set), convert to new format (Object)
...
// Consolidar todos los items únicos del refugio
function consolidateHideoutItems() {
  ...
  consolidatedHideoutItems = Array.from(itemsMap.values()).sort((a,b) => a.name.localeCompare(b.name));
}
...
function renderHideoutItemsView() {
  ...
  const filtered = consolidatedHideoutItems.filter(item => {
    ...
    const mf = hideoutItemsFilter==='all' || 
               ...
               (hideoutItemsFilter==='missing' && status === 'none');
    return ms && mf;
  });
